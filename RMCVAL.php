...
<?php 
// RMC.php
// Receives user form data from RMCForm.php
...
//Preparing the SQL query for inserting a data record into Customer table and assigning it to a php variable
 ...
 $query = "INSERT INTO Customer VALUES ('{$email}', '{$fName}', '{$lName}', '{$sAddress}', '{$city}', '{$zip}', '{$state}')";

//mysqli_query executes the query against the database (in this case, inserting a data record into Customer table)

 $resultCust = mysqli_query($cxn,$query)
      or die ("Couldn't execute query - there is already a customer record for $email. <a href=\"https://localhost/RMCForm.php\"><br><br>Back to Form</a>" );
...
//mysqli_query executes the query against the database (in this case, inserting a data record into Orders table)
